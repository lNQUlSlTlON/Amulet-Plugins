...
# Shared block exclusion lists (vegetation, liquids, etc.)
from block_exclusions import WATER_STATES, AIR, is_vegetation
...
class Build_Railroad(wx.Panel, DefaultOperationUI):
    def __init__(
        ...
        choices1 = ["Dark Oak", "Smooth Stone", "Deepslate Tile", "Iron Block", "Cobblestone", "Stone", "Blackstone", "Polished Blackstone", "Cobbled Deepslate", "Deepslate Brick", "Polished Deepslate"]
        ...
        # Create a list of choices for the dropdown
        choices2 = ["Dark Oak", "Smooth Stone", "Deepslate Tile", "Iron Block", "Cobblestone", "Stone", "Blackstone", "Polished Blackstone", "Cobbled Deepslate", "Deepslate Brick", "Polished Deepslate"]
        ...
        # Create a label for the dropdown
        dropdown3_label = wx.StaticText(self, label="Climbing Rails (Powered/Regular):")
        ...
        # Create a label for the ballast material dropdown
        dropdown5_label = wx.StaticText(self, label="Ballast Material:")
        self._sizer.Add(dropdown5_label, 0, wx.ALL | wx.ALIGN_CENTRE_HORIZONTAL, 5)

        # Create a list of choices for the ballast material dropdown
        choices5 = ["Cobblestone", "Stone", "Blackstone", "Polished Blackstone", "Cobbled Deepslate", "Deepslate Brick", "Deepslate Tile", "Polished Deepslate"]

        # Add a Choice control for the dropdown
        self._dropdown5 = wx.Choice(self, choices=choices5)
        self._sizer.Add(self._dropdown5, 0, wx.ALL | wx.ALIGN_CENTRE_HORIZONTAL, 5)
        self._dropdown5.SetSelection(0)
    ...
    def loop_operation(self, cribbing_choice, minus1_choice, power_choice, pillar_choice, ballast_choice, coordinates, second_block_coordinates, world, dimension, placed_blocks, placed_block_iter, placed_rails, placed_rails_iter, placed_support_blocks, placed_support_block_iter, support_locations):
        continue_operation = True
        recursive = False
        # Compute the roadbed blockstate for b2b_test material matching
        BALLAST_MATERIALS_LOOP = {
            "Cobblestone": "cobblestone",
            "Stone": "stone",
            "Blackstone": "blackstone",
            "Polished Blackstone": "polished_blackstone",
            "Cobbled Deepslate": "cobbled_deepslate",
            "Deepslate Brick": "deepslate_bricks",
            "Deepslate Tile": "deepslate_tiles",
            "Polished Deepslate": "polished_deepslate",
        }
        roadbed_id = BALLAST_MATERIALS_LOOP.get(ballast_choice, "cobblestone")
        track_base_blockstate = f'universal_minecraft:{roadbed_id}'
        while continue_operation:
            ...
            back_to_back, continue_coordinates, recursive, placed_blocks, placed_block_iter = self.set_roadbase(coordinates, second_block_coordinates, block_differential, recursive, ballast_choice, world, dimension, placed_blocks, placed_block_iter)
        ...
        ## The back_to_back dict is empty if the path is N-S or E-W, and contains the coordinates of the middle diagonal block if the path is diagonal
            if back_to_back != []:
                ...
                self.b2b_test(b2b_coords, world, dimension, placed_blocks, placed_block_iter, track_base_blockstate)
        ...
        placed_ballast_inner, placed_ballast_outer = self.set_ballast(ballast_choice, coordinates, world, dimension, placed_blocks, placed_block_iter, placed_rails, placed_rails_iter)
    ...
    def _run_operation(self, _):
        ...
        ballast_choice = self._dropdown5.GetStringSelection() # get the ballast material choice from the dropdown
        ...
        def operation():
            ...
            self.loop_operation(cribbing_choice, minus1_choice, power_choice, pillar_choice, ballast_choice, coordinates, second_block_coordinates, world, dimension, placed_blocks, placed_block_iter, placed_rails, placed_rails_iter, placed_support_blocks, placed_support_block_iter, support_locations)
...
###
    ...
    @staticmethod
    def b2b_test(b2b_coords, world, dimension, placed_blocks, placed_block_iter, track_base_blockstate='universal_minecraft:cobblestone'):
        # Create an array at the coordinates
            ...
            for x in range(min_x, max_x):
                for y in range(min_y, max_y):
                    ...
                    ## The debug statements below print the entire array... while "noisy" it is useful for debugging
                        ...
                        # Debug print statement for the blockstate
                        #print(block.blockstate)
                        if block.blockstate == 'universal_minecraft:wool[color=pink]' or block.blockstate == 'universal_minecraft:wool[color=orange]' or block.blockstate == 'universal_minecraft:wool[color=purple]' or block.blockstate == track_base_blockstate:
                            value = 1
                            # print('found')
...
###  The script also needs to keep track of where rails have been to exclude those checks from the next iteration.
    ...
    @staticmethod
    def set_roadbase(coordinates, second_block_coordinates, block_differential, recursive, ballast_choice, world, dimension, placed_blocks, placed_block_iter):
        back_to_back = []
        continue_coordinates = []
        direction_dict = []
        platform, version_number = world.level_wrapper.platform, world.level_wrapper.version

        # Ballast material mapping: display name -> (block_id, stair_id)
        BALLAST_MATERIALS = {
            "Cobblestone": ("cobblestone", "stone_stairs"),
            "Stone": ("stone", "normal_stone_stairs"),
            "Blackstone": ("blackstone", "blackstone_stairs"),
            "Polished Blackstone": ("polished_blackstone", "polished_blackstone_stairs"),
            "Cobbled Deepslate": ("cobbled_deepslate", "cobbled_deepslate_stairs"),
            "Deepslate Brick": ("deepslate_bricks", "deepslate_brick_stairs"),
            "Deepslate Tile": ("deepslate_tiles", "deepslate_tile_stairs"),
            "Polished Deepslate": ("polished_deepslate", "polished_deepslate_stairs"),
        }
        block_id, stair_id = BALLAST_MATERIALS.get(ballast_choice, ("cobblestone", "stone_stairs"))
        track_base_block = Block('minecraft', block_id)

        stair_northsouth_1_block = Block('minecraft', stair_id, {"upside_down_bit": ByteTag(0), "weirdo_direction": IntTag(1)})
        stair_northsouth_2_block = Block('minecraft', stair_id, {"upside_down_bit": ByteTag(0), "weirdo_direction": IntTag(0)})
        stair_eastwest_1_block = Block('minecraft', stair_id, {"upside_down_bit": ByteTag(0), "weirdo_direction": IntTag(2)})
        stair_eastwest_2_block = Block('minecraft', stair_id, {"upside_down_bit": ByteTag(0), "weirdo_direction": IntTag(3)})
...
##
    ...
    ##
        ...
        ## Continue testing at the block
            ...
            ## Use the value of the first two items in the block_differential list to determine the direction of the path 
                ...
                z2 = item[1]['z']
                value2 = item[1]['value']
                if x1 == -1 and z1 == 1 and value1 == 2 and x2 == 0 and z2 == 1 and value2 == 2:
                ...
                elif x1 == -1 and z1 == 0 and value1 == 2 and x2 == -1 and z2 == 1 and value2 == 2:
                    d1 = 'east_west'
                    d2 = 'south_east'
                    d3 = 'north_south'
                    if placed_block_iter == 1:
                ...
                elif x1 == 0 and z1 == 1 and value1 == 2 and x2 == 1 and z2 == 1 and value2 == 2:
                    d1 = 'north_south'
                    d2 = 'north_east'
                    d3 = 'east_west'
                    if placed_block_iter == 1:
                ...
                elif x1 == 1 and z1 == -1 and value1 == 2 and x2 == 1 and z2 == 0 and value2 == 2:
                #reverese the order of the coordinates to properly check the blocks in the correct order
                    x1 = item[1]['x']
                    y1 = item[1]['y']
                    z1 = item[1]['z']
                ...
                elif x1 == -1 and z1 == -1 and value1 == 2 and x2 == 0 and z2 == -1 and value2 == 2:
                #reverese the order of the coordinates to properly check the blocks in the correct order
                    x1 = item[1]['x']
                    y1 = item[1]['y']
                    z1 = item[1]['z']
    ...
    def set_rails(power_choice, coordinates, world, dimension, placed_blocks, placed_block_iter, placed_rails, placed_rails_iter):
        ...
        # Redstone block powers adjacent powered rails without needing a torch
        redstone_block = Block('minecraft', 'redstone_block')

        # Counter for consecutive ascending/descending rails
        slope_rail_count = 0
        # Deferred powered rail: when a powered rail would land on a corner,
        # delay it until the next non-corner rail
        powered_rail_deferred = False
        ...
        for i in range(len(placed_blocks)):
            ...
            is_slope = direction in ascending_directions
            is_corner = direction in invalid_powered_rail_direction

            # Track consecutive slope rails, reset on flat
            if is_slope:
                slope_rail_count += 1
            else:
                slope_rail_count = 0

            # Determine if this rail should be powered
            # Flat rails: powered every 16th rail
            # Slope rails: ALL get powered rails for momentum
            if is_slope:
                powered = True
            elif powered_rail_deferred:
                powered = True
            elif placed_rails_iter % 16 == 0:
                powered = True
            else:
                powered = False

        # Place a rail at the base block location (+1 Y) based on the direction of the path
            if powered and not is_corner:
                # Place redstone block as roadbed to power the rail above
                world.set_version_block(x1, y1, z1, dimension, (platform, version_number), redstone_block, None)
            ...
            elif powered and is_corner:
                # Can't place powered rail on a corner — defer until next straight/slope rail
                rail_block = rail_directions[direction]
                world.set_version_block(x1, y1 + 1, z1, dimension, (platform, version_number), rail_block, None)
                world.set_version_block(x1, y1 + 2, z1, dimension, (platform, version_number), air_block, None)
                placed_rails_iter += 1
                powered_rail_deferred = True
                # print('deferred powered rail at corner:', x1, y1 + 1, z1, 'direction:', direction, 'count:', placed_rails_iter)
            else:
                rail_block = rail_directions[direction]
                world.set_version_block(x1, y1 + 1, z1, dimension, (platform, version_number), rail_block, None)
                world.set_version_block(x1, y1 + 2, z1, dimension, (platform, version_number), air_block, None)
...
###
    ...
    @staticmethod
    def set_ballast(ballast_choice, coordinates, world, dimension, placed_blocks, placed_block_iter, placed_rails, placed_rails_iter):
        platform, version_number = world.level_wrapper.platform, world.level_wrapper.version
...
##
        ...
        block_id, stair_id = BALLAST_MATERIALS.get(ballast_choice, ("cobblestone", "stone_stairs"))
        stair_northsouth_1_block = Block('minecraft', stair_id, {"upside_down_bit": ByteTag(0), "weirdo_direction": IntTag(0)})
        stair_northsouth_2_block = Block('minecraft', stair_id, {"upside_down_bit": ByteTag(0), "weirdo_direction": IntTag(1)})
        stair_eastwest_1_block = Block('minecraft', stair_id, {"upside_down_bit": ByteTag(0), "weirdo_direction": IntTag(2)})
        stair_eastwest_2_block = Block('minecraft', stair_id, {"upside_down_bit": ByteTag(0), "weirdo_direction": IntTag(3)})
    ...
    ## Giant has also said that he is going to be obfuscating the "Universal" block refs, so these will need to be updated to the new format
        ...
        # Build set of path coordinates to prevent ballast from overwriting roadbed/rails
        path_coords = set()
        for block in placed_blocks:
            path_coords.add((block['x'], block['y'], block['z']))
            path_coords.add((block['x'], block['y'] + 1, block['z']))  # also protect rail level

        def safe_place(bx, by, bz, block_to_place):
            """Place a block only if it won't overwrite a path block or rail."""
            if (bx, by, bz) not in path_coords:
                world.set_version_block(bx, by, bz, dimension, (platform, version_number), block_to_place, None)
                return True
            print(f"  BLOCKED ballast at ({bx}, {by}, {bz})")
            return False
            ...
            ##
                ...
                if (direction in ['north_south', 'north_ascending', 'north_descending']) and (facing == 'north'):
                    if len(placed_blocks) == 0:
                            ...
                            safe_place(x - 1, y, z, stair_northsouth_1_block)
                            safe_place(x - 1, y + 1, z, air_block)
                            safe_place(x - 1, y + 2, z, air_block)
                            if direction == 'north_ascending' or direction == 'north_descending':
                                safe_place(x - 1, y + 3, z, air_block)
                            ...
                            safe_place(x + 1, y, z, stair_northsouth_2_block)
                            safe_place(x + 1, y + 1, z, air_block)
                            safe_place(x + 1, y + 2, z, air_block)
                            if direction == 'north_ascending' or direction == 'north_descending':
                                safe_place(x + 1, y + 3, z, air_block)
                ...
                ## East-West Facing East            
                    ...
                    if len(placed_blocks) == 0:
                        ...
                        safe_place(x, y, z - 1, stair_eastwest_1_block)
                        safe_place(x, y + 1, z - 1, air_block)
                        safe_place(x, y + 2, z - 1, air_block)
                        ...
                        safe_place(x, y, z + 1, stair_eastwest_2_block)
                        safe_place(x, y + 1, z + 1, air_block)
                        safe_place(x, y + 2, z + 1, air_block)
        ...
        ###
                ...
                ## Always Block 1
                    ...
                    ## If the next block in the sequence is not a back-to-back diagonal, set four blocks
                        if next_b2b == False:   
                            safe_place(x - 1, y, z - 1, stair_corner_7_block)
                            safe_place(x - 1, y + 1, z - 1, air_block)
                            safe_place(x - 1, y + 2, z - 1, air_block)
                            ...
                            safe_place(x + 1, y, z + 1, stair_corner_3_block)
                            safe_place(x + 1, y + 1, z + 1, air_block)
                            safe_place(x + 1, y + 2, z + 1, air_block)
            ...
            ## True for both N-NE b2b and W-SW b2b    
                ...
                ## Only Block 2 if "next back-to-back" = True and previous origin is "north-northeast"
                    if direction == 'south_east' and prev_origin == 'north-northeast':
                        ...
                        safe_place(x + 1, y + 2, z + 1, air_block)
                        placed_ballast_outer.append({'x': x + 1, 'y': y, 'z': z + 1, 'count': placed_ballast_iter, 'direction': direction, 'origin_direction': origin_direction, 'facing': facing, 'b2b': b2b, 'position': 'outer', 'state': 'diagonal', 'value': 'stair_corner_3_block'})
                        safe_place(x - 1, y, z - 1, stair_corner_6_block)
                        safe_place(x - 1, y + 1, z - 1, air_block)
                        safe_place(x - 1, y + 2, z - 1, air_block)
                ...
                ## Always Block 1                                                
                    ...
                    ## If the next block in the sequence is a back-to-back diagonal, set three blocks to accomodate the addional corner
                        ...
                        ## This block is NOT redundant in a B2B scenario
                            safe_place(x + 1, y, z + 1, stair_corner_1_block)
                            safe_place(x + 1, y + 1, z + 1, air_block)
                            safe_place(x + 1, y + 2, z + 1, air_block)
            ...
            ## True for both E-NE b2b and S-SW b2b
                ...
                ## ALways Block 0; also Block 3 if Block 1 "next back-to-back" = True, however, will be reported as Happy Path, so logic embedded in the Happy Path Test above    
                    if direction == 'east_west':
                        safe_place(x, y, z - 1, stair_northsouth_1_block)
                        safe_place(x, y + 1, z - 1, air_block)
                        safe_place(x, y + 2, z - 1, air_block)
                        placed_ballast_inner.append({'x': x, 'y': y, 'z': z - 1, 'count': placed_ballast_iter, 'direction': direction, 'origin_direction': origin_direction, 'facing': facing, 'b2b': b2b, 'position': 'inner', 'state': 'cardinal', 'value': 'stair_eastwest_1_block'})
                        placed_ballast_outer.append({'x': 'NA', 'y': 'NA', 'z': 'NA', 'count': placed_ballast_iter, 'direction': direction, 'origin_direction': origin_direction, 'facing': facing, 'b2b': b2b, 'position': 'outer', 'state': 'skip', 'value': 'skip'}) 
                ## Always Block 1                 
                    ...
                    ## If the next block in the sequence is not a back-to-back diagonal, set four blocks                        
                        if next_b2b == False:
                            safe_place(x - 1, y, z + 1, stair_corner_2_block)
                            safe_place(x - 1, y + 1, z + 1, air_block)
                            safe_place(x - 1, y + 2, z + 1, air_block)
                            ...
                            safe_place(x + 1, y, z - 1, stair_corner_8_block)
                            safe_place(x + 1, y + 1, z - 1, air_block)
                            safe_place(x + 1, y + 2, z - 1, air_block)
                ...
                ## Always Block 1   
                    ...
                    ## If the next block in the sequence is a back-to-back diagonal, set three blocks to accomodate the addional corner
                            ...
                            safe_place(x + 1, y, z - 1, air_block)
                            safe_place(x + 1, y + 1, z - 1, air_block)
                            safe_place(x + 1, y + 2, z - 1, air_block)
                ...
                ## Always Block 1                     
                    ...
                    ## If the next block in the sequence is not a back-to-back diagonal, set four blocks   
                        if next_b2b == False:
                            ...
                            safe_place(x - 1, y, z + 1, stair_corner_4_block)
                            safe_place(x - 1, y + 1, z + 1, air_block)
                            safe_place(x - 1, y + 2, z + 1, air_block)
...
###
    ...
    ###
        def test_inner_block_air_loop(placed_block_x_z_inner, air_depth, water_depth, count):
            ...
            if test_block.blockstate == AIR or is_vegetation(test_block.blockstate):
...
#
    ...
    def place_supports(cribbing_choice, minus1_choice, coordinates, world, dimension, placed_blocks, placed_block_iter, placed_rails, placed_rails_iter, placed_ballast_inner, placed_ballast_outer, placed_block_x_z_inner, placed_block_x_z_core, placed_block_x_z_outer, support_locations):
        ...
        SUPPORT_BLOCKS = {
            "Dark Oak": ("dark_oak_log", {"pillar_axis": StringTag("y")}),
        ...
        block_id, props = SUPPORT_BLOCKS.get(cribbing_choice, ("dark_oak_log", {"pillar_axis": StringTag("y")}))
        cribbing_block = Block('minecraft', block_id, props) if props else Block('minecraft', block_id)

        block_id, props = SUPPORT_BLOCKS.get(minus1_choice, ("dark_oak_log", {"pillar_axis": StringTag("y")}))
        underpinning_block = Block('minecraft', block_id, props) if props else Block('minecraft', block_id)
    ...
    # Iterate over the grouped data
        ...
        # Now we can access the consolidated data for each count
            ...
            # Check if 'Air Depths' for 'Inner', 'Core', and 'Outer' are all greater than zero
                elif all(air_depth > 1 for state, air_depth in zip(states, air_depths) if state != 'skip') or any(use_case == 'overpass' for use_case in use_cases):
                ...
                elif any((state == 'skip' and air_depth == 0) for state, air_depth in zip(states, air_depths)) and all(air_depth > 1 for state, air_depth in zip(states, air_depths) if state != 'skip'):
                    # print("All 'Air Depths' are greater than zero.")
                # Increment 'Span Length' for 'Inner', 'Core', and 'Outer'
                    data['Span']['Inner'] = True
                    data['Span']['Core'] = True
                ...
                elif all(air_depth > 1 for air_depth in air_depths) and all(water_depth > 0 for water_depth in water_depths):
