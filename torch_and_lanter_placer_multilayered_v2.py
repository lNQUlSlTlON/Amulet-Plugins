...
from block_exclusions import WATER_STATES, WATER_STATES_ALL, LAVA_STATES, GRAVEL, is_vegetation, is_non_solid, is_solid_for_lantern, is_light_fixture
...
class Detect_Edges(wx.Panel, DefaultOperationUI):
    def __init__(
        ...
        stagger_label = wx.StaticText(self, label="Placement Stagger:")
        self._sizer.Add(stagger_label, 0, wx.ALL | wx.ALIGN_CENTRE_HORIZONTAL, 5)

        # Add a SpinCtrl for stagger (randomness in placement along edge direction)
        # 0 = perfectly even spacing, higher = more random variation
        self._stagger = wx.SpinCtrl(self, min=0, max=6, initial=0)
        self._sizer.Add(self._stagger, 0, wx.ALL | wx.ALIGN_CENTRE_HORIZONTAL, 5)
    ...
    def _run_operation(self, _):
        ...
        stagger = min(self._stagger.GetValue(), max(detect_radius - 1, 0))  # cap stagger to radius - 1
        ...
        def operation():
            ...
            for y in y_values:
                ...
                # print(coordinates)
                lighting_coordinates = self.detect_edges(coordinates, block_choice, detect_radius, stagger, self.world, self.canvas.dimension)
    ...
    @staticmethod
    def detect_edges(coordinates, block_choice, detect_radius, stagger, world, dimension):
        ...
        for block in true_blocks:
            ...
            operator = block[5]
            offset = int(block[6])
            # Apply stagger: randomly offset placement along the edge direction
            # If torch faces X (east/west), edge runs along Z — stagger Z
            # If torch faces Z (north/south), edge runs along X — stagger X
            if stagger > 0:
                stagger_offset = random.randint(-stagger, stagger)
                if axis == "x":
                    block_z += stagger_offset
                elif axis == "z":
                    block_x += stagger_offset
            ...
            if place_torch:
                ...
                # Skip if this position already has a light fixture (duplicate edge or previous Y layer)
                existing_block = world.get_block(block_x, block_y, block_z, dimension)
                if is_light_fixture(existing_block.blockstate):
                    continue

                if block_choice == "lantern":
                    # Lanterns should not be suspended in mid-air, so we need to check for solid blocks above and below
                    ...
                    # Also check for lights directly adjacent vertically to prevent cross-Y stacking
                    light_above = is_light_fixture(air_test_up.blockstate)
                    light_below = is_light_fixture(air_test_down.blockstate)
                    # If there is a solid block above (not existing light) AND no light below, place a hanging lantern.
                    if is_solid_for_lantern(air_test_up.blockstate) and not light_below:
                    ...
                    # If there is a solid block below (not existing light) AND no light above, place a standing lantern.
                    elif is_solid_for_lantern(air_test_down.blockstate) and not light_above:
                ...
                if block_choice == "torch":
                    # Torches should not be placed on water, lava, gravel, or vegetation
                    if axis == "x" and operator == "+":
                        state_test = world.get_block(block_x + 1, block_y, block_z, dimension)
                        if is_non_solid(state_test.blockstate) or state_test.blockstate == GRAVEL:
                ...
                if block_choice == "both":
                    air_test_up = world.get_block(block_x, block_y + 1, block_z, dimension)
                    air_test_down = world.get_block(block_x, block_y - 1, block_z, dimension)
                    # Check for lights directly adjacent vertically to prevent cross-Y stacking
                    light_above = is_light_fixture(air_test_up.blockstate)
                    light_below = is_light_fixture(air_test_down.blockstate)
                    # Priority 1: If there is a solid block above (not existing light) AND no light below, place a hanging lantern.
                    if is_solid_for_lantern(air_test_up.blockstate) and not light_below:
                        world.set_version_block(block_x, block_y, block_z, dimension, (platform, version_number), lantern_block_hanging, None)
                        # Increment the counter variable
                        torch_placement_count += 1
                        ...
                        # Priority 2: Try wall torch — prefer torch on a solid wall over standing lantern
                        torch_placed = False
                        if axis == "x" and operator == "+":
                            state_test = world.get_block(block_x + 1, block_y, block_z, dimension)
                            if not is_non_solid(state_test.blockstate) and state_test.blockstate != GRAVEL:
                                world.set_version_block(block_x, block_y, block_z, dimension, (platform, version_number), torch_block, None)
                                torch_placement_count += 1
                                block_logging.append({'status': 'torch placed', 'x': block_x, 'y': block_y, 'z': block_z, 'blockstate': facing, 'count': torch_placement_count})
                                torch_placed = True
                            ...
                            if not is_non_solid(state_test.blockstate) and state_test.blockstate != GRAVEL:
                                ...
                                block_logging.append({'status': 'torch placed', 'x': block_x, 'y': block_y, 'z': block_z, 'blockstate': facing, 'count': torch_placement_count})
                                torch_placed = True
                        # Priority 3: Fallback — standing lantern if solid block below, no torch placed, and no light above
                        if not torch_placed and is_solid_for_lantern(air_test_down.blockstate) and not light_above:
                            world.set_version_block(block_x, block_y, block_z, dimension, (platform, version_number), lantern_block_standing, None)
                            torch_placement_count += 1
                            block_logging.append({'status': 'lantern placed', 'x': block_x, 'y': block_y, 'z': block_z, 'blockstate': 'standing', 'count': torch_placement_count})
