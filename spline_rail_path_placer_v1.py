...
class SplineRailPathPlacer(wx.Panel, DefaultOperationUI):
    ...
    def enforce_path_constraints(
        ...
        # --- 9a. Vertical Stacking Prevention ---
        # No two consecutive voxels with same X,Z and only Y difference
        i = 0
        while i < len(fixed) - 1:
            curr = fixed[i]
            nxt = fixed[i + 1]
            ...
            if dx == 0 and dz == 0 and dy != 0:
                # Pure vertical — insert a horizontal offset block
                # Pick a cardinal direction: prefer continuing the path direction
                ...
                if i > 0:
                    ...
                    pdz = curr[2] - prev[2]
                    if pdx != 0:
                        offset_x, offset_z = (1 if pdx > 0 else -1), 0
                    elif pdz != 0:
                        offset_x, offset_z = 0, (1 if pdz > 0 else -1)
                # Insert intermediate block: same Y as curr, offset horizontally
                mid = (curr[0] + offset_x, curr[1], curr[2] + offset_z)
                # Move nxt to be adjacent to mid with the elevation change
                new_nxt = (mid[0], nxt[1], mid[2])
                fixed[i + 1] = new_nxt
                # print(f"  9a: vertical stack at [{i}] {curr} -> {nxt}, inserted mid={mid}, moved nxt to {new_nxt}")
        ...
        if stats["vertical_stacks_fixed"]:
            _log_path("AFTER 9a (vertical stacking)")
...
export = {
    "name": "Spline Rail Path Placer",
    "operation": SplineRailPathPlacer,
}
