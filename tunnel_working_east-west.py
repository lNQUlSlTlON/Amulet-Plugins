...
class Dig_Tunnel(wx.Panel, DefaultOperationUI):
    ...
    def _run_operation(self, _):
        ...
        z_tunnel_widths = {0: 2, 1: 4, 2: 5, 3: 6, 4: 6, 5: 6, 6: 6, 7: 5, 8: 4, 9: 2}  # mapping of Y levels to Z widths in "Y-level: Z-width" format
        ...
        z_tunnel_widths = {0: 2, 1: 4, 2: 5, 3: 6, 4: 6, 5: 6, 6: 6, 7: 5, 8: 4, 9: 2}  # mapping of Y levels to X widths in "Y-level: Z-width" format
        for box in selection_group:
            for x in range(num_iterations):  # iterate over the X direction
        ...
        air_selection_group = []
        z_air_widths = {0: 0, 1: 0, 2: 0, 3: 5, 4: 5, 5: 5, 6: 5, 7: 3, 8: 2, 9: 0}  # mapping of Y levels to Z widths in "Y-level: Z-width" format
        for box in selection_group:
        ...
        for box in selection_group:
            for x in range(num_iterations):  # iterate over the X direction
                y = 3  # fixed Y level
                z_extension_1 = 1  # fixed Z extension
        ...
        for box in selection_group:
            for x in range(num_iterations):  # iterate over the X direction
                y = 3  # fixed Y level
                z_extension_3 = 3  # fixed Z extension
                stair2_selection_group.append(
                    SelectionBox((box.min[0] + x, y + box.min[1], box.min[2] + z_extension_3), (box.max[0] + x, y + box.min[1] + 1, box.max[2] + z_extension_3))
                )
                z_extension_4 = -1  # fixed X extension
                stair2_selection_group.append(
                    SelectionBox((box.min[0] + x, y + box.min[1], box.min[2] + z_extension_4), (box.max[0] + x, y + box.min[1] + 1, box.max[2] + z_extension_4))
                )                
        ...
        for box in selection_group:
            for x in range(num_iterations):  # iterate over the X direction
                y = 3  # fixed Y level
                z_extension_5 = -5  # fixed Z extension
                stair3_selection_group.append(
                    SelectionBox((box.min[0] + x, y + box.min[1], box.min[2] + z_extension_5), (box.max[0] + x, y + box.min[1] + 1, box.max[2] + z_extension_5))
                )
        ...
        for box in selection_group:
            for x in range(num_iterations):  # iterate over the X direction
                ...
                z_extension_6 = -2  # fixed Z extension
                stair4_selection_group.append(
                    SelectionBox((box.min[0] + x, y + box.min[1], box.min[2] + z_extension_6), (box.max[0] + x, y + box.min[1] + 1, box.max[2] + z_extension_6))
               )
        ...
        for box in selection_group:
            for x in range(num_iterations):  # iterate over the X direction
                y = 3  # fixed Y level
                z_extension_7 = 5  # fixed Z extension
                stair5_selection_group.append(
                    SelectionBox((box.min[0] + x, y + box.min[1], box.min[2] + z_extension_7), (box.max[0] + x, y + box.min[1] + 1, box.max[2] + z_extension_7))
                )                
        ...
        for box in selection_group:
            for x in range(num_iterations):  # iterate over the X direction
                ...
                y = 8  # fixed Y level, upper
                z_extension_8 = 2  # fixed Z extension
                stair6_selection_group.append(
                    SelectionBox((box.min[0] + x, y + box.min[1], box.min[2] + z_extension_8), (box.max[0] + x, y + box.min[1] + 1, box.max[2] + z_extension_8))
               )
        ...
        for box in selection_group:
            for x in range(num_iterations):  # iterate over the X direction
                y = 3  # fixed Y level
                z_extension_3 = 2  # fixed Z extension
                track_base_selection_group.append(
                    SelectionBox((box.min[0] + x, y + box.min[1], box.min[2] + z_extension_3), (box.max[0] + x, y + box.min[1] + 1, box.max[2] + z_extension_3))
                )
                z_extension_4 = -2  # fixed X extension
                track_base_selection_group.append(
                    SelectionBox((box.min[0] + x, y + box.min[1], box.min[2] + z_extension_4), (box.max[0] + x, y + box.min[1] + 1, box.max[2] + z_extension_4))
                )    
        ...
        for box in selection_group:
            for x in range(num_iterations):  # iterate over the X direction
                ...
                z_extension_4 = -2  # fixed Z extension
                rails_selection_group.append(
                    SelectionBox((box.min[0] + x, y + box.min[1], box.min[2] + z_extension_4), (box.max[0] + x, y + box.min[1] + 1, box.max[2] + z_extension_4))
                )   
    ...
    def stair1_at_selection_coordinates(world: "BaseLevel", dimension: Dimension, selection: SelectionGroup):
        ...
        stair1_block = Block('minecraft', 'stone_stairs', {"upside_down_bit": ByteTag(0), "weirdo_direction": IntTag(2)})
        # get the platform and version number of the game
        platform, version_number = world.level_wrapper.platform, world.level_wrapper.version
        for box in selection.selection_boxes:
            for z in range(box.min[2], box.max[2]):
    ...
    def stair2_at_selection_coordinates(world: "BaseLevel", dimension: Dimension, selection: SelectionGroup):
        ...
        stair2_block = Block('minecraft', 'stone_stairs', {"upside_down_bit": ByteTag(0), "weirdo_direction": IntTag(3)})
        # get the platform and version number of the game
        platform, version_number = world.level_wrapper.platform, world.level_wrapper.version
        for box in selection.selection_boxes:
            for x in range(box.min[0], box.max[0]):
    ...
    def stair3_at_selection_coordinates(world: "BaseLevel", dimension: Dimension, selection: SelectionGroup):
        ...
        stair3_block = Block('minecraft', 'deepslate_tile_stairs', {"upside_down_bit": ByteTag(0), "weirdo_direction": IntTag(3)})
        # get the platform and version number of the game
        platform, version_number = world.level_wrapper.platform, world.level_wrapper.version
        for box in selection.selection_boxes:
            for x in range(box.min[0], box.max[0]):
    ...
    def stair4_at_selection_coordinates(world: "BaseLevel", dimension: Dimension, selection: SelectionGroup):
        ...
        stair4_block = Block('minecraft', 'deepslate_tile_stairs', {"upside_down_bit": ByteTag(1), "weirdo_direction": IntTag(3)})
        # get the platform and version number of the game
        platform, version_number = world.level_wrapper.platform, world.level_wrapper.version
        for box in selection.selection_boxes:
            for x in range(box.min[0], box.max[0]):
    ...
    def stair5_at_selection_coordinates(world: "BaseLevel", dimension: Dimension, selection: SelectionGroup):
        ...
        stair5_block = Block('minecraft', 'deepslate_tile_stairs', {"upside_down_bit": ByteTag(0), "weirdo_direction": IntTag(2)})
        # get the platform and version number of the game
        platform, version_number = world.level_wrapper.platform, world.level_wrapper.version
        for box in selection.selection_boxes:
            for x in range(box.min[0], box.max[0]):
    ...
    def stair6_at_selection_coordinates(world: "BaseLevel", dimension: Dimension, selection: SelectionGroup):
        ...
        stair6_block = Block('minecraft', 'deepslate_tile_stairs', {"upside_down_bit": ByteTag(1), "weirdo_direction": IntTag(2)})
        # get the platform and version number of the game
        platform, version_number = world.level_wrapper.platform, world.level_wrapper.version
        for box in selection.selection_boxes:
            for x in range(box.min[0], box.max[0]):
    ...
    def rails_at_selection_coordinates(world: "BaseLevel", dimension: Dimension, selection: SelectionGroup):
        ...
        rail_block = Block('minecraft', 'rail', {"rail_direction": IntTag(1)})
        # get the platform and version number of the game
        platform, version_number = world.level_wrapper.platform, world.level_wrapper.version
        for box in selection.selection_boxes:
            for y in range(box.min[1], box.max[1]):
...
export = {
    "name": "Tunnel East-West (+X)",  # the name of the plugin
