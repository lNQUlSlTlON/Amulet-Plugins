...
class SplineRailPathPlacerV3(wx.Panel, DefaultOperationUI):
    def __init__(
        self,
        parent: wx.Window,
        canvas: "EditCanvas",
    ...
    def _on_scan(self, _):
        """Phase 1: Scan selection for markers, find control points."""
        ...
        self._set_status(
            f"Scan complete! Found {len(control_pts)} control point(s). "
            f"Click Preview to compute path."
        )

    def _on_preview(self, _):
        """Phase 2: Compute path from spline and show as selection preview."""
        ...
        self._set_status(
            ...
            f"Adjust tension and re-Preview, or click Fill."
        )
    ...
    def _on_tension_change(self, _):
        """Recompute and re-preview when tension slider changes."""
        if not self._scan_complete or len(self._ordered_points) < 2:
            return
...
export = {
    "name": "Spline Rail Path Placer v3",
    "operation": SplineRailPathPlacerV3,
}
