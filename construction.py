...
class ConstructionReader:
    def __init__(self, file_or_buffer: Union[str, IO]):
        ...
        if isinstance(file_or_buffer, str):
            ...
            file_or_buffer = file_or_buffer.replace("\\", "/")  # Added this line to fix Windows path issue
    ...
    ## Debug Print Statement
        # print("Initialized ConstructionReader with buffer:", self._buffer)
...
##
## This is the function that seemed to need the most debugging
##
    ...
    ## Debug Print Statement -- Construction Version should be 0    
        # print("Format version:", self._format_version)
        ...
        ## Debug Print Statement
            # print("Reading with buffer:", self._buffer)

            buffer = self._buffer.read(metadata_end - metadata_start)
        ## Debug Print Statement            
            # print("Metadata in Binary:", buffer)

            try:
                metadata = amulet_nbt.load(buffer, compressed=True)
                # print('Metadata after unpacking:', metadata)
...
### Debugging
###
            #print(f"Read function - File path: {self._buffer.name}")  # print file path
            #print(f"Read function - len {length} bytes")  # print how many bytes you are going to read
            #print(f"Reading (len) Buffer: {self._buffer.read(length)}")  # print the buffer
            #print(f"Reading (Entire) Buffer: {self._buffer.read()}")  # print the buffer            

            data = self._buffer.read(length)
            # print(f"Read function - Data: {data}")  # print the data

            nbt_obj = amulet_nbt.load(data)

            # print(f"Read function - NBT Object: {nbt_obj}")  # print the nbt object
